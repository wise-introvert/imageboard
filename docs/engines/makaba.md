...
Раньше была возможность возвращать ответ в формате JSON при передаче параметра `json` со значением `1`. После обновления API в августе 2022-ого, судя по всему, данную возможность не перенесли в новый API, поэтому формат ответа — HTML. Формат HTML — такой же как в разделе ["Поиск по архиву"](#поиск-по-архиву).

<!--
<details>
<summary>Пример ответа в формате JSON. <code>board: "a"</code> <code>text: "гештальт"</code></summary>

...
Для отправки сообщения в тред оправляется `POST` запрос по адресу `https://2ch.hk/makaba/posting.fcgi` с данными формы:
...
Для отправки жалобы на тред или комментарий отправляется `POST` запрос по адресу `/makaba/makaba.fcgi?json=1` с параметрами:

* `task` — `"report"`
* `board` — ID доски.
* `thread` — ID треда.
* `posts` — ID комментария. В случае жалобы на тред совпадает с ID треда. Название параметра — во множественном числе, так как раньше можно было пожаловаться сразу на несколько комментариев, отметив их галками.
...
Для отправки голоса за сообщение отправляется `GET` запрос по адресу `https://2ch.hk/api/<действие>?board=<id-доски>&num=<id-поста>`, где `<действие>` — это либо `like`, либо `dislike`.
...
"Архив" ведёт себя достаточно странно: какие-то треды архивируются, какие-то — просто "тонут" и не появляются в архиве. Возможно, треды с малым количеством комментариев просто отбрасываются как "мусорные", и потому не архивируются. Также часто бывает задержка (например, около часа) между тем как тред "потонул" и его появлением в архиве.
...
Создание нового треда или отправка комментария в существующий тред.

Описание параметров можно найти в описании ["старого" API постинга](#постинг).
...
* `board` — Доска.
* `thread` — ID треда. Если не указано, будет создан тред.
* `name` — Имя автора.
* `email` — Электронная почта автора.
* `tags` — Теги треда (при создании треда).
* `subject` — Тема комментария (или треда).
* `comment` — Текст комментария.
* `icon` — Значок комментария (или треда).
* `op_mark` — Поставить ли метку "ОП треда" при отправке комментария в тред.
* `file[]` — Список вложений.

Пример ответа без ошибки при создании нового треда:

```js
{
  result: 1, // Код ответа. "0" — "ошибка".
  thread: 194338603 // ID треда.
}
```

Пример ответа без ошибки при отправке комментария в тред:

```js
{
  result: 1, // Код ответа. "0" — "ошибка".
  num: 194338603 // ID поста.
}
```
...
`GET` `https://2ch.hk/api/<действие>?board=<доска>&num=<id-поста>`

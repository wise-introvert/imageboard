...
"Архив" ведёт себя достаточно странно: какие-то треды архивируются, какие-то — просто "тонут" и не появляются в архиве. Также часто бывает задержка (например, около часа) между тем как тред "потонул" и его появлением в архиве.

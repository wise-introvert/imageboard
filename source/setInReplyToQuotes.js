 ...
 * @param {boolean} [options.generateBlockQuotes] — Is `true` by default. Passing `generateBlockQuotes: false` could seem like not making any sense, but it is used in `captchan` when parsing comments not starting from the first one: in that case it only parses a few of the "previous" comments which are then used in autogenerated quotes for the comments being shown; to further reduce the parsing time, autogenerated quotes for those few "previous" comments aren't generated by passing `generateBlockQuotes: false`, further reducing the overall parsing time by a little bit.
